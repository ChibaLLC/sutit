...
const props = defineProps({
  starter: {
    type: Object as PropType<typeof data.value>,
    required: false
  }
})
...
const data = ref<FormData>(props.starter || {
  name: '',
  ...
  <Modal :show="showPriceModal" @confirm="showPriceModal = false" @cancel="showPriceModal = false"
    ...
    <div>
      ...
      <input type="number" id="payment-amount"
             ...
             placeholder="Amount to charge for the form" v-model="data.payment.amount">
    </div>
    <div class="mt-4">
      ...
      <div class="flex flex-col justify-start mt-2">
        ...
        <small class="text-red-500 font-mulish -mt-2"
               v-if="data.requireMerch && !Object.values(data.formData.stores).at(0)?.length">Be sure
          to
          add
          an
    ...
    <div v-if="data.allowGroups" class="mt-4">
      <label for="group-payment-amount">Group Amount Payable
        <input type="number" id="group-payment-amount"
               ...
               placeholder="Amount to charge a group for the form" v-model="data.payment.group_amount">
      </label>
      <label for="group-member-limit">
        Group Member Number Limit
        <input type="number" v-model="data.payment.group_limit" id="group-member-limit"
               class="border-1 border-solid px-3 py-3 placeholder-gray-400 text-gray-700 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full mt-2">
  ...
  <Modal :show="showFormNameModal" @cancel="closeFormDetailsModal" @confirm="closeFormDetailsModal"
    ...
    <div>
      ...
      <input type="text" id="form-name"
             ...
             placeholder="Name of the form (Required)" v-model="data.name">
    ...
    <div class="mt-4">
      ...
      <textarea id="form-description"
                class="border-1 border-solid px-3 py-3 placeholder-gray-400 text-gray-700 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full mt-1"
                placeholder="Description of the form" v-model="data.description"></textarea>
    </div>
