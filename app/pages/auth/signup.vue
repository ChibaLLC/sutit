    ...
    async onResponse({ response }) {
      ...
      switch (res.statusCode) {
        ...
        case Status.conflict:
          window.alertError("A user with that very email, already exists.", { timeout: 'never' })
          break
        default:
          window.alertError(res?.body?.error || res?.body?.message || res?.body || "An unknown error occurred. Please try again. Later.", { timeout: 'never' })
      }
    }
  })
}
  ...
  <div
    ...
    <form @submit.prevent="submit" class="flex flex-col gap-8 max-sm:gap-4">
      ...
      <div class="flex flex-col gap-1">
        ...
        <input type="password" id="password1" autocomplete="new-password" v-model="details.password1"
          class="rounded-md px-3 py-2 focus:ring-1 outline-none focus:ring-sky focus:ring-opacity-50 border border-[#bdc6d7]">
      </div>
      <div class="flex flex-col gap-1 -mt-4">
        <label for="password">Confirm Password</label>
        <input type="password" id="password2" autocomplete="new-password" v-model="details.password2"
          class="rounded-md px-3 py-2 focus:ring-1 outline-none focus:ring-sky focus:ring-opacity-50 border border-[#bdc6d7]">
      ...
      <div class="flex justify-center gap-x-6 items-center w-full -mt-2">
        <button type="button" :disabled="loadingGoogle" @click="clickGoogleBtn"
        ...
        <button type="button" @click="loginWithGithub" :disabled="loadingGithub"
          class="bg-white rounded-md px-4 py-2 border border-[#bdc6d7] hover:bg-[#e8f0fe] transition-colors">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
