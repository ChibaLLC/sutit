...
function submit() {
    ...
    $fetch(`/api/v1/auth/reset?email=${email}&token=${token}`, {
        method: 'POST',
                            ...
                            <form>
                                ...
                                <div class="relative w-full mb-3">
                                    ...
                                    <input type="password"
                                        ...
                                        ref="password" v-on:focus="clearErrors" autocomplete="new-password"
                                        style="transition: all 0.15s ease 0s;" />
                                </div>
                                <div class="relative w-full mb-3">
                                    <label class="block uppercase text-gray-700 text-xs font-bold mb-2"
                                        for="grid-password">Password Again</label>
                                    <input type="password"
                                        class="border-0 px-3 py-3 placeholder-gray-400 text-gray-700 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full"
                                        v-model="details.password" v-on:focus="clearErrors" autocomplete="new-password"
                                        style="transition: all 0.15s ease 0s;" />
                                </div>
                                <div class="text-center mt-6">
                                    <button
                                        ...
                                        <span :class="{ 'loading': loading }" class="w-full grid place-items-center"
                                            v-else>
