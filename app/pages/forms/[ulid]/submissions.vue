...
function getData(pile: {
  ...
  return pile.map(pages => {
    return {
      fields: getFields(pages.response || {}).map(data => {
        if (isCheckbox(data.type)) {
          data.value = Object.values(data.value || {}).map(d => capitalize(d as string))
        }
        return data
      }),
      meta: pages.meta
...
<template>
  ...
  <div class="flex min-h-screen w-full">
    <main class="w-full flex flex-col items-center max-w-[1200px] mx-auto">
      <div class="mt-4 w-full flex justify-between items-center px-10">
        ...
        <div class="flex gap-1">
          ...
          <button
            ...
            :class="{ 'cursor-not-allowed': loadingCheckout }" :disabled="loadingCheckout">
            Credit
      ...
      <div class="w-full mt-2 px-10 relative overflow-x-auto rounded">
        <table
          ...
          <tbody class="divide-y divide-slate-200 text-white border-b">
            <tr v-for="(data, index) in getData(responses as any)"
              class="text-sm text-slate-700 cursor-pointer hover:bg-slate-200">
              <td class="px-6 py-4">{{ index + 1 }}</td>
              <td v-for="column of data.fields" class="max-w-[30px]">
                <div v-html="Array.isArray(column.value) ?
                  `<div class='text-ellipsis self-start hover:max-h-fit'>${column.value.join('<br>')}</div>`
                  : column.value"
      ...
      <Modal title="Choose a Credit Method" :show="showCreditMethodsModal" @confirm="showCreditMethodsModal = false"
