...
function hasPrice(
  form: Omit<Drizzle.Form.select, "pages"> & { pages: Forms },
): boolean {
...
function hasPhone() {
  return (
    payment_details.value.token || payment_details.value.phone.length >= 10
  );
}
...
async function processForm() {
  ...
  if (hasPrice(data.forms) && !hasPhone()) {
...
class Invite<
  T extends { email: string } | { phone: string } = any,
> extends Set<T> {
...
function clearText() {
  ...
  for (const node of element.childNodes) {
    if (
      ...
      node.nodeValue?.includes(inviteTexts.value)
    ) {
      node.remove();
    }
  }
...
<template>
  ...
  <div class="flex min-h-screen">
    <div
      class="flex flex-col p-2 w-full max-w-[820px] ml-auto mr-auto shadow-2xl h-fit mt-4 rounded-md"
    >
      <div class="header">
        ...
        <p
          class="bg-slate-700 w-full px-4 py-2 pl-12 rounded"
          v-if="data.forms?.formDescription"
        >
          {{ data.forms?.formDescription }}
        </p>
      </div>
      <form
        class="pb-4 mt-2 min-h-max"
        @submit.prevent
        v-if="!data?.forms?.allowGroups || group.self"
      >
        <FormViewer
          :data="formStoreData"
          @submit="completeForm"
        ...
        />
        <div
          class="flex w-full px-4 ml-0.5 relative justify-between flex-wrap gap-2 mt-2"
        >
          <small
            class="text-gray-500 w-fit"
            v-if="data.forms.price_individual > 0 && !token"
          >
            This form requires payment for submission of <br />
            <span class="text-red-400"
              >Amount Due: {{ data.forms.price_individual }}</span
            >
            KES
          </small>
          <div v-if="complete">
            <button
              v-if="complete"
              @click="goBack"
              class="bg-slate-700 text-white rounded px-4 py-2 mr-2"
            >
              Back
            </button>
            <button
      ...
      <form
        @submit.prevent
        v-if="data?.forms?.allowGroups && !group.chosen"
      ...
      >
        ...
        <div class="mt-4">
          ...
          <br />
          <small class="ml-0.5"
            >Add the phone numbers or emails for your members.</small
          >
          <div class="relative">
            <div
              @keydown.enter.prevent="addPhoneOrEmail"
              contenteditable="true"
              @keyup.,.prevent="addPhoneOrEmail"
              id="members"
              ...
              @focusout="showPlaceholder"
              @input="addText"
            >
              <span class="placeholder" v-if="!hidePlaceholder">
                Example: allan@gmail.com, 0712345678, +254712345678,
                boni@mail.com, etc. <br />
                Don't forget yourself. 😀
              </span>
              <br v-if="invites.size > 0" />
              <div
                v-for="invite in invites"
                class="bg-navy text-white rounded p-0.5 invite mt-1"
                :key="invite.email"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
            ...
            <small class="help has-text-grey"
              >Press <code>Enter</code> or <code>,</code> to add a
              contact</small
            >
          </div>
        </div>
        ...
        >
          <small
            class="text-gray-500 w-fit"
            v-if="data.forms.price_group_amount > 0"
          >
            This form requires payment for submission of <br />
            <span class="text-red-400"
              >Amount Due: {{ data.forms.price_group_amount }}</span
            >
            KES
          </small>
        </div>
