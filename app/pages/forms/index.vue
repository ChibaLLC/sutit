...
const { data: formsStores } = await useFetch("/api/v1/forms/me", {
  headers: {
...
function deleteForm(ulid: string) {
  const form = formsStores.value?.find((fs) => fs.form_meta.ulid === ulid)
  formsStores.value = formsStores.value?.filter((fs) => fs.form_meta.ulid !== ulid)
  $fetch(`/api/v1/forms/${ulid}/delete`, {
    ...
    onResponse({ response }) {
      ...
      formsStores.value?.push(form as any)
    },
      ...
      <div class="w-full px-8 mt-5 relative overflow-x-auto rounded-t">
        ...
        <table class="w-full text-sm text-left rtl:text-righ rounded bg-gradient-to-br from-white to-slate-100 text-slate-900 mt-1"
          ...
          <tbody class="divide-y divide-gray-200 border-b">
            <tr v-for="(formStore, index) in formsStores" :key="formStore.form_meta.ulid"
              class="text-sm cursor-pointer hover:bg-slate-200"
              @click="navigateIfTarget($event, `/forms/${formStore.form_meta.ulid}/submissions`)">
              <td class="px-6 py-4 border-r border-gray-100">
                {{ index + 1 }}
              </td>
              <td class="px-6 py-4">
                {{ formStore.form_meta.formName }}
              </td>
              <td class="px-6 py-4">
                {{ formStore.form_meta.formDescription }}
              </td>
              <td class="px-6 py-4">
                {{ new Date(formStore.form_meta.createdAt).toLocaleString() }}
              </td>
              <td class="px-6 py-4">
                {{ formStore.form_meta.price_individual ? 'Yes' : 'No' }}
              </td>
              <td class="px-6 py-4">
                <div class="flex justify-center gap-1 items-center h-full">
                  <NuxtLink :to="`/forms/${formStore.form_meta.ulid}`" title="view">
                  ...
                  <span class="rounded" @click.prevent="getShareableLink(formStore.form_meta.ulid)" title="share">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-5 h-5" fill="currentColor"
                      :id="`copy_link_${formStore.form_meta.ulid}`">
                      <path
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                      :id="`copy_success_${formStore.form_meta.ulid}`" class="w-5 h-5  hidden">
                      <path
                        d="M9.9997 15.1709L19.1921 5.97852L20.6063 7.39273L9.9997 17.9993L3.63574 11.6354L5.04996 10.2212L9.9997 15.1709Z">
                      </path>
                    </svg>
                  </span>
                  <NuxtLink :to="`/forms/${formStore.form_meta.ulid}/edit`" title="edit">
                  ...
                  <span @click="deleteForm(formStore.form_meta.ulid)">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
