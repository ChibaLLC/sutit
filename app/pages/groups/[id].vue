<script setup lang="ts">
  ...
  const form = computed(() => groupData.value?.data?.form);
  const members = computed(() => groupData.value?.data?.members || []);
  const stats = computed(() => groupData.value?.data?.stats);
  const paymentSummary = computed(() => groupData.value?.data?.paymentSummary);
  ...
  const copyInviteLink = async () => {
    try {
      await navigator.clipboard.writeText(groupData.value?.data?.inviteLink || "");
    ...
    <div class="container mx-auto max-w-6xl px-4 py-8">
      <div class="mb-8">
        <div class="text-muted-foreground mb-2 flex items-center gap-2 text-sm">
          <NuxtLink :to="`/forms/${groupData?.data?.formId}`" class="hover:underline">
            {{ form?.title }}
          </NuxtLink>
          <span>/</span>
          <NuxtLink to="/groups" class="hover:underline">Groups</NuxtLink>
          <span>/</span>
