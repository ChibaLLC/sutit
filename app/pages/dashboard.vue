...
type CardData = {
  ...
  title: string;
  count: number;
  description: string; // Can Have Html format
  icon: Component
};
...
cards.value = [
  {
    ...
    count: stats?.forms || 0,
    url: "/forms",
    description: `<span>You have created</span><span class="font-medium text-gray-900 mx-1">${stats?.forms || 0}</span>
              <span>forms so far</span>`,
    icon: FileTextIcon,
  },
  {
    title: "MONEY",
    count: stats?.earnings || 0,
    url: "/finance",
    description: `<span>You have earned</span>
<span class="font-medium text-gray-900 mx-1">${stats?.earnings || 0} KES</span>
              <span>so far</span>`,
    icon: DollarSignIcon
  },
  {
    title: "REACH",
    count: stats?.responses || 0,
    url: "/finance",
    description: `<span>You have reached</span>
<span class="font-medium text-gray-900 mx-1">${stats?.responses || 0}</span>
              <span>people so far</span>`,
    icon: GitGraphIcon
  },
];
...
<template>
  ...
  <div class="container lg:flex-nowrap flex-wrap">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
      ...
      <NuxtLink
        ...
        :key="card.url"
        :to="card.url"
        class="bg-white rounded-xl shadow-sm hover:shadow-md transition-all duration-300 border border-gray-500 overflow-hidden group"
      >
        <div class="p-6">
          <div class="flex items-center justify-between">
            ...
            <div
              class="bg-gray-100 group-hover:bg-gray-200 rounded-full p-3 transition-colors"
            >
              <component :is="card.icon" class="w-6 h-6 text-gray-700" />
            </div>
          </div>
  ...
  <div class="flex flex-col px-4 xl:px-6 mt-8 w-full">
    <h1
      class="w-full xl:w-6/12 xl:px-5 lg:px-10 lg:w-full md:w-9/12 m-auto uppercase font-bold mt-1"
    >
      Recents
    </h1>
    <ul
    ...
    >
      <li
        v-for="item in recents?.forms"
        class="flex align-middle gap-3 m-auto p-2 w-full"
      >
        <div class="w-fit">
          <svg
    ...
    <div
      v-else
      class="w-full xl:w-6/12 xl:px-5 lg:px-10 lg:w-full md:w-9/12 m-auto mt-2"
    >
      <p class="text-center text-gray-500">
        You have not created any forms yet.
        <NuxtLink to="/forms/create" class="text-blue-500"
  ...
  <div class="flex flex-col px-4 xl:px-6 mt-8">
    ...
    <ul
      class="w-full xl:w-6/12 xl:px-5 lg:px-10 lg:w-full md:w-9/12 m-auto mt-2 text-center"
    >
      <li
        class="py-2 bg-emerald-700 text-white rounded hover:bg-emerald-600 transition-colors w-40 font-semibold uppercase"
