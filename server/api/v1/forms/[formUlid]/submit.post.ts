import { insertData, invalidateFromGroupLink, needsIndividualPayment } from "../utils/queries";
...
export default defineEventHandler(async (event) => {
	...
	const [data, needsPay] = await needsIndividualPayment(formUlid, _data.forms.price_individual).catch(
		(err) => [err as Error, false] as [Error, boolean]
	);
	...
	if (needsPay && !_data.phone && !_data.token)
	...
	async function insert(
		...
		sendUserMail(
			{ email: creator!.email },
			`New response on form ${data.forms.formName}`,
			`[Update] Submission ${data.forms.formName}`
		);
		if (formMail) {
			sendUserMail(
				{ email: formMail },
				`Form submission successful for ${data.forms.formName}`,
				`[Update] Successful form submission ${data.forms.formName}`
			);
		}
	...
	if (needsPay && !_data.token) {
		...
		return await processFormPayments(
			...
			(paymentUlid: string) => {
				...
				sendUserMail(
					{ email: creator?.email },
					`${_data.phone} has paid KES: ${_data.forms.price_individual}.00 for your form ${data.forms.formName}`,
					`[Payment]: Confirmed payment on ${data.forms.formName}`
				);
				...
				if (formMail) {
					...
					sendUserMail(
						{ email: formMail },
						`Payment successful for ${data.forms.formName}`,
						`[Update]: Payment Successful ${data.forms.formName}`
					);
				}
			}
		).catch((err) => {
		...
		invalidateFromGroupLink(token.token, result.body!);
		return result;
	} else if (!needsPay) {
