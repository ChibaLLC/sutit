...
export default defineEventHandler(async (event) => {
	...
	const { ResultParameters, ResultType, OriginatorConversationID, ConversationID, TransactionID, ReferenceData } =
		Result;
	if (ResultType !== 0) {
	...
	return await insertWithdrawal(+amount, transactionCode.toString());
});
